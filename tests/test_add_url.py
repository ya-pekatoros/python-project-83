...
def test_add_url(client, test_app):
    with mock.patch('psycopg2.connect', autospec=True) as mock_connect:
        ...
        mock_cur.execute.assert_any_call('INSERT INTO urls (name, created_at) VALUES (%s, %s) RETURNING id',
                                         ('https://google.com', today_iso))
        flash_mes = 'Страница успешно добавлена'.encode("utf-8", "ignore")
        assert flash_mes in response.data
        assert today.strftime('%d-%m-%Y').encode("utf-8", "ignore") in response.data

        mock_cur.fetchone.return_value = ('1', "https://asus.com", datetime.datetime.strptime("26-01-2023", "%d-%m-%Y"))
        response = client.post('/urls', data={
            'url': "https://asus.com",
        }, follow_redirects=True)
